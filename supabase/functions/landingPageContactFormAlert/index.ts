...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Send notification to contact@jayd.ai
    ...
    if (!contactEmailResponse.ok) {
      ...
      return new Response("Erreur SendGrid lors de l'envoi de la notification", {
        status: 500,
        headers: corsHeaders
      })
    }
    return new Response("Notification de contact envoyée", {
      status: 200,
      headers: corsHeaders
    })
  } catch (error) {
                ...
                <table cellpadding="0" cellspacing="0" border="0" align="center" style="margin: 0 auto 30px; width: 100%; max-width: 280px;">
                  <tr>
                    <td align="center" style="background-color: rgba(103, 106, 221, 0.1); border-radius: 12px; border: 1px solid rgba(103, 106, 221, 0.15);" class="tutorial-bg">
                      <a href="https://www.youtube.com/watch?v=Igf3FxmZpJw" style="display: block; padding: 12px 10px; font-family: 'Roboto', Arial, sans-serif; font-size: 16px; color: #676ADD; text-decoration: none; font-weight: 600; white-space: nowrap; text-overflow: ellipsis; overflow: hidden;" class="tutorial-text">
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
      ...
      <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin-top: 20px;">
        ...
        <p><strong>Entreprise :</strong> ${record.company || 'Non spécifié'}</p>
