...
async def update_data_collection(
    request: Request,
    update_data: UpdateDataCollectionDTO
) -> Response:
    try:
        user_id = request.state.user_id
        logger.info(
            f"User {user_id} updating data collection preference to {update_data.data_collection}"
        )

        UserService.update_data_collection(
            request.state.supabase_client,
            user_id,
            update_data.data_collection
        )
        ...
        raise HTTPException(
            ...
            detail=f"Failed to update data collection: {str(e)}"
        )
