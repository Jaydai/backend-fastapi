...
@router.patch("/{invitation_id}/status", response_model=InvitationResponseDTO, status_code=status.HTTP_200_OK)
async def update_invitation_status(
    request: Request, invitation_id: str, update_data: UpdateInvitationStatusDTO
    ...
    try:
        ...
        invitation = InvitationService.update_invitation_status(
            request.state.supabase_client, invitation_id, user_id, update_data.status
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update invitation: {str(e)}"
        )
