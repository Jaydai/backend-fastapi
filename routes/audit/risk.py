...
async def get_organization_risk_stats(
    ...
    logger.info(f"[AUDIT:risk] Request started - org_id={organization_id}, start_date={start_date}, end_date={end_date}, days={days}")
    ...
    try:
        ...
        logger.info(f"[AUDIT:risk] Calling AuditService.get_organization_risk_stats...")
        ...
        logger.info(f"[AUDIT:risk] Request completed - org_id={organization_id}, duration={duration_ms}ms, has_data={result is not None}")
        ...
        duration_ms = int((time.time() - start_time) * 1000)
        logger.error(f"[AUDIT:risk] Error getting risk stats - org_id={organization_id}, duration={duration_ms}ms, error={str(e)}", exc_info=True)
