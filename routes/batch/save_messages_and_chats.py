...
async def save_messages_and_chats(request: Request, batch_data: CombinedBatchDTO) -> CombinedBatchResponseDTO:
    try:
        ...
        logger.info(f"User {user_id} saving combined batch: {messages_count} messages, {chats_count} chats")
        ...
        if batch_data.messages:
            message_responses, message_result = MessageService.save_messages_batch(
                request.state.supabase_client, user_id, batch_data.messages
            )
            message_stats = {
        ...
        if batch_data.chats:
            chat_responses, chat_result = ChatService.save_chats_batch(
                request.state.supabase_client, user_id, batch_data.chats
            )
            chat_stats = {
        ...
        return CombinedBatchResponseDTO(
            messages=message_responses, chats=chat_responses, stats={"messages": message_stats, "chats": chat_stats}
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to save combined batch: {str(e)}"
        )
