...
async def get_ai_coach_insights(
    ...
    try:
        ...
        org_members_response = client.table("organization_members").select(
            "user_id"
        ...
        chats_response = client.table("enriched_chats").select(
        ...
        recent_quality = sum(quality_scores[:mid_point]) / mid_point if mid_point > 0 and len(quality_scores) >= mid_point else 0
        older_quality = sum(quality_scores[mid_point:]) / (len(quality_scores) - mid_point) if len(quality_scores) > mid_point else 0
        ...
        theme_distribution = [
            {
                "theme": theme,
                "count": count,
                "percentage": round((count / total_prompts) * 100, 1)
            }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get AI coach insights: {str(e)}"
        )
