...
@router.get("/{organization_id}", response_model=OrganizationDetailResponseDTO)
@require_permission_in_organization(PermissionEnum.ORGANIZATION_READ)
async def get_organization_by_id(request: Request, organization_id: str) -> OrganizationDetailResponseDTO:
    try:
        user_id = request.state.user_id
        logger.info(f"User {user_id} fetching details for organization {organization_id}")

        organization = OrganizationService.get_organization_by_id(request.state.supabase_client, organization_id)
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to fetch organization: {str(e)}"
        )
