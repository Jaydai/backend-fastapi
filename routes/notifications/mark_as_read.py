...
from dtos.notification_dto import UpdateNotificationDTO, MarkAllReadResponseDTO
...
async def mark_all_notifications_as_read(
    request: Request,
    mark_all_as_read: bool = Query(False)
) -> MarkAllReadResponseDTO:
    try:
        if not mark_all_as_read:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Query parameter 'mark_all_as_read=true' is required"
            )
...
async def mark_notification_as_read(
    request: Request,
    notification_id: int,
    update_data: UpdateNotificationDTO
):
    try:
        user_id = request.state.user_id
