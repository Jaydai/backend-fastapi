...
class NotificationRepository:
    ...
    def get_all_notifications(client: Client, user_id: str) -> list[Notification]:
        response = client.table("notifications") \
            .select("*") \
            .eq("user_id", user_id) \
    ...
    def get_notification_counts(client: Client, user_id: str) -> tuple[int, int]:
        total_response = client.table("notifications") \
            .select("id", count="exact") \
            .eq("user_id", user_id) \
            .execute()

        unread_response = client.table("notifications") \
            .select("id", count="exact") \
            .eq("user_id", user_id) \
            .is_("read_at", "null") \
            .execute()
    ...
    def mark_notification_as_read(client: Client, notification_id: int, user_id: str) -> bool:
        verification = client.table("notifications") \
            .select("id") \
    ...
    def mark_all_notifications_as_read(client: Client, user_id: str) -> int:
        unread = client.table("notifications") \
            .select("id") \
            .eq("user_id", user_id) \
            .is_("read_at", "null") \
            .execute()
