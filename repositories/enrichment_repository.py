...
class EnrichmentRepository:
    ...
    @staticmethod
    def save_enriched_message(
        client: Client, user_id: str, enriched_message: EnrichedMessage
    ...
    def get_rated_chats(
        ...
        query = (
            client.table("enriched_chats").select("*").eq("user_id", user_id).gte("created_at", cutoff_date.isoformat())
        )
    ...
    def override_chat_quality(client: Client, user_id: str, chat_provider_id: str, quality_score: int) -> bool:
        ...
        response = (
            client.table("enriched_chats")
            .update({"user_override_quality": True, "user_quality_score": quality_score})
            .eq("user_id", user_id)
            .eq("chat_provider_id", chat_provider_id)
            .execute()
        )
