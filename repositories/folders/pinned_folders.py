...
def get_pinned_folder_ids(client: Client, user_id: str) -> list[str]:
    ...
    response = client.table("users_metadata").select("pinned_folder_ids").eq("user_id", user_id).single().execute()
...
def pin_folder(client: Client, user_id: str, folder_id: str) -> list[str]:
    ...
    client.table("users_metadata").update({"pinned_folder_ids": current_pinned}).eq("user_id", user_id).execute()
...
def update_pinned_folders(client: Client, user_id: str, folder_ids: list[str]) -> list[str]:
    ...
    client.table("users_metadata").update({"pinned_folder_ids": folder_ids}).eq("user_id", user_id).execute()
