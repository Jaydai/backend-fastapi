...
class AuthRepository:
    ...
    def sign_in_with_password(sign_in_dto: SignInDTO) -> Session:
        ...
        return Session(
            access_token=sign_in_response.session.access_token, refresh_token=sign_in_response.session.refresh_token
        )

    @staticmethod
    def oauth_sign_in(sign_in_dto: OAuthSignIn) -> Session:
        sign_in_response = supabase.auth.sign_in_with_id_token(
            {"provider": sign_in_dto.provider, "token": sign_in_dto.token}
        )
        return Session(
            access_token=sign_in_response.session.access_token, refresh_token=sign_in_response.session.refresh_token
        )
