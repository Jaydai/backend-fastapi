...
class MessageService:
    ...
    @staticmethod
    def save_messages_batch(client: Client, user_id: str, messages: list[SaveMessageDTO]) -> tuple[list[MessageResponseDTO], MessageBatchResultDTO]:
        if not messages:
            return [], MessageBatchResultDTO(
                saved_count=0,
                skipped_count=0,
                total_count=0
            )
    ...
    def _convert_timestamp(timestamp: float | None) -> str:
        if timestamp:
            try:
                ...
                return datetime.fromtimestamp(timestamp_in_seconds, tz=timezone.utc).isoformat()
            except Exception:
...
class ChatService:
    ...
    def save_chat(client: Client, user_id: str, chat_dto: SaveChatDTO) -> ChatResponseDTO:
        existing_chat = ChatRepository.get_chat_by_provider_id(
            client,
            user_id,
            chat_dto.chat_provider_id
        )
        ...
        if existing_chat:
            ...
            updated_chat = ChatRepository.update_chat(
                client,
                user_id,
                chat_dto.chat_provider_id,
                update_data
    ...
    @staticmethod
    def save_chats_batch(client: Client, user_id: str, chats: list[SaveChatDTO]) -> tuple[list[ChatResponseDTO], ChatBatchResultDTO]:
        if not chats:
            return [], ChatBatchResultDTO(
                inserted_count=0,
                updated_count=0,
                total_count=0
            )
        ...
        result_dto = ChatBatchResultDTO(
            inserted_count=len(inserted_chats),
